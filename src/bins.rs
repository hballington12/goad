...
pub fn generate_theta_phi_combinations() -> Vec<(f32, f32)> {
    let thetas = Array1::linspace(0.0, std::f32::consts::PI, 306).insert_axis(ndarray::Axis(1)); // Reshape to (50, 1)

    let phis = Array1::linspace(0.0, 2.0 * std::f32::consts::PI, 306).insert_axis(ndarray::Axis(0)); // Reshape to (1, 60)
